...
#---------------------------------------------------------------------------------
#---------------------------------------------------------------------------------
# read the noisefree data

noise_free_recons = np.zeros_like(mean_recons)

for i, gf in enumerate(df.gradient_factor.cat.categories):
    fpath = res_dir / f'noisefree_gf{gf}_SquaredL2Norm.h5'
    with h5py.File(fpath, 'r') as f:
        noise_free_recons[i, ...] = f['recons'][:]
...
for i_gf, gf in enumerate(df.gradient_factor.cat.categories):
    for i_b, beta in enumerate(betas):
        ax[i_gf, i_b].plot(np.abs(mean_recons[i_gf, i_b, :]), lw=3 * lw)
        ax[i_gf, i_b].plot(np.abs(noise_free_recons[i_gf, i_b, :]), lw=lw)
for i_b, beta in enumerate(betas):
